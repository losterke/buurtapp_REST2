...
function weZijnIngelogd() {
    FB.api('/me', function(response) {
        console.log("foto ophalen");
  $("#profile_pic").attr("src","https://graph.facebook.com/"+response.id+"/picture");
        console.log("naam ophalen");
  $('.profile_info').append("<strong>"+response.name+"</strong><br><small>25</small>");
          console.log("profiel pagina aanpassen");
  $("#profile_pict").attr("src","https://graph.facebook.com/"+response.id+"/picture");
  
  $('.profile_Page').append("<h2>" + response.name + "</h2><br>"
          +'<img src="https://graph.facebook.com/' 
          + response.id + '/picture">'+"<br>naam: "
          + response.last_name+"<br>voornaam: "
          + response.first_name 
          +"<br>e-mail: "
          + response.email);
  });
